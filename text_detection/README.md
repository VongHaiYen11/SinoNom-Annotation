...
Single-image inference passes `palette='random'` to MMDetection 3.3.0, which
skips building the test dataset solely to obtain visualization colors. No
`FS_Dataset` or `HDR_Dataset` registration or top-level `mmdet/` source copy
is needed. The model, checkpoint, test transforms and bbox/score processing
are unchanged; JSON output does not use the visualization palette.
MMDetection itself must still be installed. If an external config explicitly
imports `mmdet.datasets.fssj`, `mmdet.datasets.hdr`, or
`text_detection.runtime.datasets` through `custom_imports`, remove only those
dataset imports for inference. Preserve imports for custom models/transforms.
